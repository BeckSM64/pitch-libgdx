...
{
    //Stack because cards will be thrown on top of each other, like a stack ;p
    ...
    /*
     * Compare card about to be added to pile to see if it beats cards in collection
     * Needs to consider current suit also
     */
    ...
    public boolean hasTrumpJack(String trump)
    {
        for(int i = 0; i < this.size(); i++)
        {
            ...
            //Look for card that has trump suit and value is 11
            //11 represents jack because it's one after 10
    ...
    public Card getLowestTrump(String trump)
    {
        ...
        {
            Card currentCard = this.getCard(i);
            if((this.getCard(i).getSuit() == trump
                    && lowestTrump == null)
                    || (currentCard.getSuit() == trump
                    && currentCard.getValue() < lowestTrump.getValue()))
            {
    ...
    public Card getHighestTrump(String trump)
    {
        ...
        for(int i = 0; i < this.size(); i++)
        {
            ...
            if((this.getCard(i).getSuit() == trump
                    && highestTrump == null)
                    || (currentCard.getSuit() == trump
                    && currentCard.getValue() > highestTrump.getValue()))
            {
